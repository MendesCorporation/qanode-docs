...
- Syntax highlighting
- Autocomplete for `steps`, `variables`, `files`, `web`, `mobile`, `page`, `app`, `return`, `console.log`
- Syntax validation
...
| `files` | `object` | Safe reading and writing of execution files |
...
The `files` namespace can read `fileRef` values from previous nodes and create new files as execution artifacts.

| Method | Description |
|--------|-------------|
| `files.read(fileRef)` | Reads a file and returns `{ name, mimeType, sizeBytes, buffer }` |
| `files.readText(fileRef, encoding?)` | Reads a file as text |
| `files.write({ name, mimeType?, data })` | Creates a file and returns `fileRef` |
| `files.writeText(name, text, mimeType?)` | Creates a text file and returns `fileRef` |

Files created by Custom JavaScript are saved only in QANode artifact storage. The code does not write to arbitrary filesystem paths.
...
| Maximum total size per node | 50 MB |
| File name | Automatically sanitized |
| MIME type | Inferred when omitted |
| Dangerous extensions | Blocked |
| Executable/script MIME types | Blocked |

Extensions such as `.exe`, `.sh`, `.bat`, `.cmd`, `.js`, `.jar`, `.apk`, `.dmg`, `.msi`, `.docm`, `.xlsm`, and equivalents cannot be generated by Custom JavaScript.
...
### Create text file with `files.writeText`
...
The return value of these methods is a `fileRef`, ready to be used in nodes such as **HTTP Request**, **Extract File**, **SSH Command**, **Mobile Flow**, or components.
