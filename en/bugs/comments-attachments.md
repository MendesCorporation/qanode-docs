...
| Action through MCP | Shows when a change was made through an MCP integration |

### Bug Attachments vs. Run Artifacts

Bug attachments are files intentionally added to the bug record. Run artifacts are evidence generated by the original execution, such as screenshots, logs, downloads, or reports.

When a bug is opened from a failed run, the original run remains linked to the bug. Users and integrations with permission can inspect the run artifacts without duplicating every file as a bug attachment.
