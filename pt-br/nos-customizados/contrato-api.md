...
> No exemplo acima, `timeoutMs: 600000` define um timeout de 10 minutos para este nó. Se omitido, o padrão é 5 minutos.
...
| `timeoutMs` | `number` | Não | Timeout de execução em milissegundos (padrão: 300000 = 5 minutos) |
...
| `POST /execute` | 5 minutos (padrão) — configurável via `timeoutMs` no manifesto |
...
No exemplo acima, o nó terá um timeout de 30 minutos (1.800.000ms). Se `timeoutMs` não for definido, o padrão é **5 minutos** (300.000ms).
...
7. **Timeout**: Use `timeoutMs` no manifesto para nós que precisam de mais de 5 minutos. Implemente também timeouts internos para evitar que operações fiquem penduradas
